...
# Route for handling confirmations
...
def generate_description(booking_counts, confirmed_counts):
    ...
    high_growth_threshold = 1  # arbitrary threshold for high growth rate
...
def statistics():
    ...
    def generate_plot():
        ...
        plt.plot(months, booking_counts, marker='o', color='orange', label='Bookings')
        plt.plot(months, confirmed_counts, marker='o', color='green', label='Confirmed')
        plt.xlabel('Month')
        plt.ylabel('Bookings')
        plt.title('Monthly Growth of Bookings and Confirmed Entries')
        plt.legend()
        plt.grid(True)
        plt.xticks(np.arange(1, 13), ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
        plt.yticks(np.arange(1, max(max(booking_counts), max(confirmed_counts)) + 1))
        plt.tight_layout()

        # Construct the file path using the specified directory
        graph_file = os.path.join(save_directory, 'monthly_growth.png')
...
def get_image_locations():
    ...
    formatted_locations = [(item[0], item[1].replace('\\', '/').replace('static/', '')) for item in image_locations]
    cursor.close()
    db.close()
